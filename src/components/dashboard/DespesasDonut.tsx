...
export function DespesasDonut({ mes }: { mes: string }) {
  const { data, isLoading } = useDreMes(mes);
  const rows = data
    ? [
        { name: "Operacionais", value: Math.abs(Number(data.total_despesas_operacionais)) },
        { name: "Comerciais/Marketing", value: Math.abs(Number(data.total_despesas_comerciais_marketing)) },
        { name: "Administrativas", value: Math.abs(Number(data.total_despesas_administrativas)) },
        { name: "Financeiras", value: Math.abs(Number(data.total_despesas_financeiras)) },
      ].filter((r) => r.value > 0)
    : [];
  ...
  return (
    <Card>
      <CardHeader title="Despesas por categoria" subtitle={`DRE — ${formatMesLabel(mes)}`} />
