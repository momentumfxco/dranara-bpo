    ...
    <div className="flex h-full flex-col px-2">
      ...
      {rows.length === 0 ? (
        ...
        <div className="flex flex-1 flex-col justify-center overflow-y-auto">
          <ul className="mx-auto w-full max-w-md space-y-5">
            {rows.map((r) => {
              const pct = bruta ? (r.value / bruta) * 100 : 0;
              return (
                <li
                  key={r.label}
                  className="grid grid-cols-[136px_minmax(0,1fr)_92px_44px] items-center gap-x-4"
                >
                  ...
                  <span
                    className={`text-right text-xs tabular-nums ${r.value < 0 ? "text-destructive" : "text-foreground"}`}
                  >
                    {BRL2.format(r.value)}
                  </span>
      ...
      <p className="mt-3 border-t border-dashed border-border pt-3 text-center text-[11px] text-muted-foreground">
